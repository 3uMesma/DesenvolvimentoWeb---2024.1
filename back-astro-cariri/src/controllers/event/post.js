...
exports.postEvent = async (req, res, next) => {
  try {
    ...
    if (!tipo) {
      return res
        .status(400)
        .json({ error: "Tipo de evento não pode ser nulo" });
    }
    ...
    const query =
      "INSERT INTO Event_(title, requester, contact, institution, address, event_type_id, date_, description) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *";
    const values = [
      titulo,
      interessado,
      contato,
    ...
    return res
      .status(200)
      .json({ message: "Evento adicionado com sucesso!", event: newEvent });
  } catch (error) {
    next(error);
  }
};
